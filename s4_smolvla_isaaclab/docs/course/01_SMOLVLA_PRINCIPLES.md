...
| Control FPS | 120 Hz |
| 图像 | 3 路 680×480 RGB |
| Dataset ID | `local/s4_drawer_insert_close_v1_10phase` |
...
| 相机内外参 | 抽屉初始开度（`drawer_initial_open.enabled=true`） |
| 工作室光照 | |
...
当前任务通过固定光照、三路相机、已启用的主罐分层格内随机、抽屉初始开度和可选的
远离抓取区干扰物构造数据变化。当前默认启用主罐 XY 随机、关闭干扰物；运动学可达
