...
def test_active_rollout_config_has_consistent_timing_and_safety() -> None:
    ...
    assert cfg.robot["rollout"]["execute_horizon"] == 20
    assert cfg.robot["rollout"]["replan_interval_steps"] == 10
    assert cfg.robot["freshness"]["max_response_age_ms"] == 500
    assert cfg.robot["freshness"]["max_motion_chunk_age_ms"] == 1200
    assert cfg.robot["freshness"]["max_chunk_age_ms"] == 1800
    ...
    assert cfg.robot["safety"]["max_rollout_joint_velocity_rad_s"] == 0.25
    assert cfg.robot["safety"]["max_rollout_joint_acceleration_rad_s2"] == 0.75
    assert cfg.robot["safety"]["chunk_blend_duration_ms"] == 100
