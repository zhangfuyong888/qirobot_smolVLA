...
def test_active_task_config_resolves_paths():
    ...
    assert config.dataset.repo_id == "local/s4_drawer_insert_close_v4_12phase_serial_acquire"
    assert training["dataset"] == "s4_drawer_insert_close_v4_12phase_serial_acquire"
    assert training["language_contract_version"] == "drawer_12phase_v4_serial_acquire"
    assert str(config.training.output_dir).endswith("smolvla_drawer_insert_close_v4_12phase_serial_acquire")
